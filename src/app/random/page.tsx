...
const randomGenres = [
  ...
  "r&b", "country", "indie", "metal", "folk", "latin"
];
...
export default function RandomPage() {
  ...
  const fetchRandomTracks = async () => {
    ...
    try {
      const response = await fetch(`/api/search?q=genre:${genre}&type=track&limit=20`);
      if (response.ok) {
      ...
      <main className="flex-1 pt-24 pb-12 px-4 max-w-6xl mx-auto w-full">
        <div className="text-center mb-8">
          ...
          <p className="text-white/50 mb-4">
            {currentGenre && `Exploring: ${currentGenre.charAt(0).toUpperCase() + currentGenre.slice(1)}`}
          </p>
          <button
            onClick={fetchRandomTracks}
            disabled={loading}
            className="inline-flex items-center gap-2 px-6 py-3 bg-[#1db954] hover:bg-[#1aa34a] disabled:bg-gray-600 text-white font-semibold rounded-full transition-colors"
          >
